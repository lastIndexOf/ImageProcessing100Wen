...
class DIP:
    ...
    def sobel_filter(self, img, k_size=3):
        H, W = img.shape
...
if __name__ == '__main__':
    ...
    dip = DIP()
    out_v, out_h = dip.sobel_filter(dip.gray_img(img))

    cv2.imwrite(os.path.join(dirname, 'imori_out15_v.jpg'), out_v)
    cv2.imwrite(os.path.join(dirname, 'imori_out15_h.jpg'), out_h)
    cv2.imwrite(os.path.join(dirname, 'imori_out15.jpg'), out_h+out_v)
