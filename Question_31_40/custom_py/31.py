...
class DIP:
    def affine_transform(self, img, x_scale=1., y_scale=1., rotate=0, x_translate=0, y_translate=0, x_skew=0, y_skew=0):
        H, W, C = img.shape

        # 空间变换
        E = np.array([
            [x_scale * cos(rotate), y_skew - sin(rotate), x_translate],
            [x_skew + sin(rotate), y_scale * cos(rotate), y_translate],
            [0, 0, 1]]
        )
...
if __name__ == '__main__':
    ...
    out = dip.affine_transform(img, x_skew=0.3)
    cv2.imwrite(path.join(dirname, './imori_out31.jpg'), out)
